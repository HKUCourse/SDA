...
def shrink_loss(pseudo_label, logits_u_s, conf_thresh):
    ...
    if mask.mean().item() == 1: # no uncertain samples to shrink
        # loss_u_shrink_batch = F.cross_entropy(sorted_logits_s, torch.zeros(B).long().cuda(), reduction='none').mean()
        loss_u_shrink_batch = 0
    else:
        for b in range(B):
            ...
            for c in range(2, C):
                ...
                if (sub_conf >= conf_thresh) or (c == C - 1):
                    ...
                    # for our loss re-weighting principle 1
...
def train(model, optimizer, scheduler, dltrain_x, dltrain_u, epoch, n_iters_per_epoch, local_rank, certain_ratio_ema, rank, logger):
    ...
    removed_class_idx = AverageMeter()
    mask_ratio = AverageMeter()
    losses_shrink = AverageMeter()
    losses_shrink_weighted = AverageMeter()
    ...
    for i in range(n_iters_per_epoch):
        ...
        # for our loss re-weighting principle 2
        ...
        if rank == 0 and i % 100 == 0:
            logger.info('Epoch {}, Iter: {}/{}, Time: {:.3f} ({:.3f}), Data: {:.3f} ({:.3f}), Loss x: {:.3f} ({:.3f}), Loss u: {:.3f} ({:.3f}), '
                        'Loss shrink: {:.3f} ({:.3f}), Loss shrink re-weighted: {:.3f} ({:.3f}), Removed class idx: {:.2f} ({:.2f}), '
                        'Mask ratio: {:.3f} ({:.3f})'.format(epoch, i, n_iters_per_epoch, batch_time.val, batch_time.avg, data_time.val, data_time.avg,
                                                             losses_x.val, losses_x.avg, losses_u.val, losses_u.avg, losses_shrink.val, losses_shrink.avg,
                                                             losses_shrink_weighted.val, losses_shrink_weighted.avg, removed_class_idx.val,
                                                             removed_class_idx.avg, mask_ratio.val, mask_ratio.avg))
